(function() {
  ...
  function updateHeader(agent) {
    ...
    if (agent && agent.name) {
      ...
      let avatarUrl = null;
      if (agent.avatar_url) {
        avatarUrl = agent.avatar_url.startsWith('http') 
          ? agent.avatar_url 
      ...
      if (agentInfo?.avatar_url) {
        aiAvatarUrl = agentInfo.avatar_url.startsWith('http')
          ? agentInfo.avatar_url
          : `${apiUrl.replace('/api', '')}${agentInfo.avatar_url}`;
      }
      
      logoContainer.innerHTML = aiAvatarUrl
        ? `<img src="${aiAvatarUrl}" alt="Agent" style="width:100%;height:100%;object-fit:cover;border-radius:50%;" />`
